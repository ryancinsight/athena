//! `BiCGSTAB` per-iteration driver behind [`super::BiCgStab`]: dimension
//! validation, shadow-residual seeding, and the per-iteration step with its
//! termination tests. The public entry points in `algorithm` assemble an
//! [`Execution`] and drive it through [`Execution::initialize`] and
//! [`Execution::iterate`].
    ...
    Obs: IterationObserver<B::Scalar>,
{
    ...
    pub(super) fn initialize(
        &mut self,
    ) -> ExecutionResult<B, Stage<BiCgStabState<B::Scalar>, B::Scalar>> {
        let rhs_norm = self
            .backend
...
pub(super) struct BiCgStabState<T> {
    initial_residual: T,
    last_residual: T,
    threshold: T,
    rho: T,
